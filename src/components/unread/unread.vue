<template>
  <div class='unread'>
    <div class="newmes">
      <div class="newmesTitle">新消息({{hasnot_read.length}})
        <span @click="read">全部已读</span>
      </div>
      <ul class="newUl" v-for="(hasNo, index) in hasnot_read" :key="index">
        <li :data-id='hasNo.topic.id' @click="U_Details($event)" :key="hasNo.topic.id">
          <div>来自 {{hasNo.author.loginname}}
            <span>18分钟前</span>
          </div>
    ...
    <div class="readmes">
      ...
      <ul class="newUl readmesUL" v-for="(hasItem, indexe) in has_read" :key="indexe">
        <li :data-id='hasItem.topic.id' @click="U_Details($event)" :key="hasItem.topic.id">
          <div>来自 {{hasItem.author.loginname}}
            <span>18分钟前</span>
          </div>
